 ...
 */
@Autonomous
public class StoneNparkRed extends LinearOpMode {
...
//            moveDistance(0.4, 700);
        ...
        sleep(1000);
        // drive backward
        encoderDriveIMU(DRIVE_SPEED,-4,-4,5);
        ...
        // drive to second stone position
        // if the left stone or right position go to the right position due to the fact we cannot collect second left stone
        if (leftvaluedetect == 0 || rightvaluedetect == 0) {
            encoderDriveIMU(DRIVE_SPEED,-28,-28,10);

        }
        // drive for middle stone
        else {
            encoderDriveIMU(DRIVE_SPEED,-37,-37,10);
        }



        // turn towards skystone / stone
        encoderDrive(TURN_SPEED, -17.5,17.5,5);
        // move back so claw does not collide with stones
        encoderDriveIMU(DRIVE_SPEED,-28,-28,5);
        // deploy claw
        // arm up
        liftPosition(0.7,16,8);
        // arm position to out
        armIn();
        sleep(750);
        // arm down
        liftPosition(0.7,-16,5);
        // strafe the rest of the distance
        encoderStrafe(DRIVE_SPEED,25,-25,5);
        // drive forward into stone
        ...
        // turn to sky bridge
        encoderDrive(TURN_SPEED,16.5,-16.5,5);
        // drive through sky bridge
        encoderDriveIMU(DRIVE_SPEED,55,55,10);
        // drop stone
        openClaw();
        // park on line

        encoderDriveIMU(DRIVE_SPEED,-10,-10,5);
        // strafe against skybridge
        encoderStrafe(1,10,-10,5);
